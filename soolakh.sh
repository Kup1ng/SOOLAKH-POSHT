...
;;
arm64|aarch64)
PRIMARY_URL="http://127.0.0.1/soolakh_premium_linux_arm64.tar.gz"
...
sleep 1
if ! curl -sSL --max-time 10 -o "$DOWNLOAD_DIR/soolakh.tar.gz" "$PRIMARY_URL"; then
echo -e "${YELLOW}Primary download failed or timed out. Trying fallback URL...${NC}\n"
if ! curl -sSL --max-time 30 -o "$DOWNLOAD_DIR/soolakh.tar.gz" "$FALLBACK_URL"; then
echo -e "${RED}Both download attempts failed.${NC}"
rm -rf "$DOWNLOAD_DIR"
exit 1
fi
fi
echo -e "Extracting Soolakh...\n"
sleep 1
mkdir -p "$config_dir"
tar -xzf "$DOWNLOAD_DIR/soolakh.tar.gz" -C "$config_dir"
echo -e "${GREEN}Soolakh installation completed.${NC}\n"
...
if [[ -f "${config_dir}/soolakh_premium" ]]; then
echo -e "Core Version: ${YELLOW}$($config_dir/soolakh_premium -v)${GREEN}"
fi
}
display_server_info() {
echo -e "\e[93m═══════════════════════════════════════════\e[0m"
...
echo -e "\n${RED}Soolakh-Core directory not found. Install it first through 'Install Soolakh core' option.${NC}\n"
...
echo
local transport=""
while [[ ! "$transport" =~ ^(tcp|tcpmux|utcpmux|ws|wsmux|uwsmux|udp|tcptun|faketcptun|xtcpmux|xwsmux)$ ]]; do
echo -ne "[*] Transport type (tcp/tcpmux/utcpmux/ws/wsmux/uwsmux/udp/tcptun/faketcptun/xtcpmux/xwsmux): "
read -r transport
if [[ ! "$transport" =~ ^(tcp|tcpmux|utcpmux|ws|wsmux|uwsmux|udp|tcptun|faketcptun|xtcpmux|xwsmux)$ ]]; then
colorize red "Invalid transport type. Please choose from tcp, tcpmux, utcpmux, ws, wsmux, uwsmux, udp, tcptun, faketcptun, xtcpmux, xwsmux."
echo
fi
done
echo
local tun_name="soolakh"
if [[ "$transport" == "tcptun" || "$transport" == "faketcptun" ]]; then
while true; do
echo -ne "[-] TUN Device Name (default soolakh): "
read -r tun_name
if [[ -z "$tun_name" ]]; then
tun_name="soolakh"
fi
...
while true; do
echo
echo -ne "[-] Mux concurrency (default 10): "
read -r mux
if [[ -z "$mux" ]]; then
mux=10
fi
...
if [[ "$transport" =~ ^(tcpmux|wsmux|utcpmux|uwsmux|xtcpmux|xwsmux)$ ]]; then
while true; do
echo
...
echo
cat << EOF > "${service_dir}/soolakh-iran${tunnel_port}.service"
[Unit]
Description=Soolakh Iran Port $tunnel_port (Iran)
After=network.target
[Service]
Type=simple
ExecStart=${config_dir}/soolakh_premium -c ${config_dir}/iran${tunnel_port}.toml
Restart=always
RestartSec=3
[Install]
WantedBy=multi-user.target
...
if systemctl enable --now "${service_dir}/soolakh-iran${tunnel_port}.service" >/dev/null 2>&1; then
...
if [[ "$transport" =~ ^(ws|wsmux|uwsmux|xwsmux)$ ]]; then
while true; do
echo
...
max_connections = 0 # 0 for unlimited
keepalive_period = 20
nodelay = ${nodelay}
retry_interval = 3
dial_timeout = 10
...
EOF
echo
cat << EOF > "${service_dir}/soolakh-kharej${tunnel_port}.service"
[Unit]
Description=Soolakh Kharej Port $tunnel_port
After=network.target
[Service]
Type=simple
ExecStart=${config_dir}/soolakh_premium -c ${config_dir}/kharej${tunnel_port}.toml
Restart=always
RestartSec=3
[Install]
WantedBy=multi-user.target
EOF
systemctl daemon-reload >/dev/null 2>&1
if systemctl enable --now "${service_dir}/soolakh-kharej${tunnel_port}.service" >/dev/null 2>&1; then
...
colorize red "You should delete all services first and then delete the Soolakh-Core."
sleep 3
return 1
else
...
echo
colorize yellow "Do you want to remove Soolakh-Core? (y/n)"
read -r confirm
echo
if [[ $confirm == [yY] ]]; then
if [[ -d "$config_dir" ]]; then
rm -rf "$config_dir" >/dev/null 2>&1
colorize green "Soolakh-Core directory removed." bold
else
colorize red "Soolakh-Core directory not found." bold
fi
else
colorize yellow "Soolakh-Core removal canceled."
fi
echo
press_key
}
...
colorize red "No config files found in the Soolakh directory." bold
echo
press_key
return 1
fi
...
local soolakh_version=$($config_dir/soolakh_premium -v)
if [ "$file_version" != "$soolakh_version" ]; then
colorize cyan "New Core version available: $soolakh_version => $file_version" bold
fi
rm "$tmp_file"
}
...
if [ $? -eq 0 ]; then
echo -e "${GREEN}Existing $Soolakh_SCRIPT has been successfully removed from $DEST_DIR.${NC}"
else
echo -e "${RED}Failed to remove existing $Soolakh_SCRIPT from $DEST_DIR.${NC}"
sleep 1
return 1
fi
else
echo -e "${YELLOW}$Soolakh_SCRIPT does not exist in $DEST_DIR. No need to remove.${NC}"
fi
curl -s -L -o "$DEST_DIR/$Soolakh_SCRIPT" "$SCRIPT_URL"
echo
if [ $? -eq 0 ]; then
chmod +x "$DEST_DIR/$Soolakh_SCRIPT"
colorize yellow "Type 'soolakh' to run the script.\n" bold
colorize yellow "For removing script type: rm -rf /usr/bin/soolakh\n" bold
press_key
exit 0
else
echo -e "${RED}Failed to download $Soolakh_SCRIPT from $SCRIPT_URL.${NC}"
sleep 1
return 1
fi
}
...
4) hawshemi_script ;;
5) download_and_extract_soolakh "menu";;
6) update_script ;;
7) remove_core ;;
0) exit 0 ;;
*) echo -e "${RED} Invalid option!${NC}" && sleep 1 ;;
