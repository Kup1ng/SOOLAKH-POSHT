...
sleep 1
if ! curl -4 -sSL --max-time 10 -o "$DOWNLOAD_DIR/soolakh.tar.gz" "$PRIMARY_URL"; then
echo -e "${YELLOW}Primary download failed or timed out. Trying fallback URL...${NC}\n"
if ! curl -4 -sSL --max-time 30 -o "$DOWNLOAD_DIR/soolakh.tar.gz" "$FALLBACK_URL"; then
...
curl -4 --max-time 1 -s -o "$tmp_file" "$url"
if [ $? -ne 0 ]; then
...
curl -4 -s -L -o "$DEST_DIR/$Soolakh_SCRIPT" "$SCRIPT_URL"
echo
if [ $? -eq 0 ]; then
